  ...
  const addToList = async (e) => {
    ...
    if (item.trim() !== '') {
      if (items.length < maxItems) {
        try {
          // Send item to backend to add to the database
          const response = await fetch('http://localhost:3001/addIngredients', {
            method: 'POST',
            headers: {
          ...
          if (response.ok) {
            ...
            console.error('Failed to add ingredient to database');
          }
        } catch (error) {
